...
export default function AlertaRiesgosForm({ userId, userEmail }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} style={{ maxWidth: 680, margin: '0 auto', padding: '16px 16px 80px', fontFamily: 'Outfit, sans-serif' }}>
      ...
      {/* Título */}
      <div style={{ marginBottom: 12 }}>
        <h1 style={{ fontSize: 18, fontWeight: 800, color: '#0B1D3A', margin: 0 }}>Alerta de Riesgos</h1>
        <p style={{ fontSize: 12, color: '#7A90B0', margin: '4px 0 0', fontWeight: 500 }}>Identificación y recomendación de riesgos en campo</p>
      </div>

      {/* Progreso */}
      {(() => { const req = [f.placaVeh, f.cedulaGuarda, f.nombreGuarda, f.nombreCliente, f.nombrePuesto, f.identificacionRiesgo]; const p = Math.round(req.filter(Boolean).length / req.length * 100); return (
