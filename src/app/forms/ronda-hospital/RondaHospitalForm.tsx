...
interface Punto { nombre: string; hora: string; novedad: string }
...
export default function RondaHospitalForm({ userId }: Props) {
  ...
  const [f, setF] = useState({
    ...
    nombreGuardaFirma: '', observacionesGenerales: '',
  })
  const [puntos, setPuntos] = useState<Punto[]>([{ nombre: '', hora: '', novedad: '' }])
  const [fotoRonda, setFotoRonda] = useState<Photo | null>(null)
  ...
  const req = [f.placaUnidad, f.cedulaGuarda, f.nombreGuarda, f.nombreGuardaFirma]
  const progreso = Math.round(req.filter(Boolean).length / req.length * 100)

  const updPunto = (i: number, k: keyof Punto, v: string) => setPuntos(p => p.map((pt, idx) => idx === i ? { ...pt, [k]: v } : pt))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('ronda_hospital').insert({
      ...
      puntos: puntos,
      nombre_guarda_firma: f.nombreGuardaFirma,
      observaciones_generales: f.observacionesGenerales,
      fotos: { fotoRonda: fotoRonda?.name || null },
  ...
  return (
    <form onSubmit={handleSubmit} style={{ maxWidth: 680, margin: '0 auto', padding: '16px 16px 80px', fontFamily: 'Outfit, sans-serif' }}>
      ...
      <div style={{ marginBottom: 12 }}>
        <h1 style={{ fontSize: 18, fontWeight: 800, color: '#0B1D3A', margin: 0 }}>Ronda Hospital Santa Mónica</h1>
        <p style={{ fontSize: 12, color: '#7A90B0', margin: '4px 0 0', fontWeight: 500 }}>Control por puntos en instalaciones clínicas</p>
      </div>
      ...
      <Section num={1} icon="👤" title="Datos del Guarda">
        <G2>
      ...
      <Section num={2} icon="📍" title="Puntos de Ronda">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <span style={{ fontSize: 12, fontWeight: 600, color: '#7A90B0' }}>Puntos registrados: {puntos.length}</span>
          {puntos.length < 30 && (
            <button type="button" onClick={() => setPuntos(p => [...p, { nombre: '', hora: '', novedad: '' }])}
        ...
        {puntos.map((pt, i) => (
          <div key={i} style={{ background: '#F4F1EB', border: '1.5px solid #D0D9E8', borderRadius: 10, padding: 12, marginBottom: 10 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: '#0B1D3A', marginBottom: 10 }}>Punto {i + 1}</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 8 }}>
              <Field label="Nombre del Punto">
                <input style={inp} value={pt.nombre} onChange={e => updPunto(i, 'nombre', e.target.value)} placeholder="Ej: Urgencias" />
              </Field>
              <Field label="Hora">
                <input style={inp} type="time" value={pt.hora} onChange={e => updPunto(i, 'hora', e.target.value)} />
              </Field>
            </div>
            <Field label="Novedad">
              <textarea style={{ ...ta, minHeight: 56 }} value={pt.novedad} onChange={e => updPunto(i, 'novedad', e.target.value)} placeholder="Sin novedad" />
            </Field>
          </div>
        ))}
      </Section>

      <Section num={3} icon="✍️" title="Cierre de Ronda">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <Field label="Nombre Guarda (Firma)" req>
            <input style={inp} value={f.nombreGuardaFirma} onChange={set('nombreGuardaFirma')} placeholder="Nombre completo" />
          </Field>
          <Field label="Observaciones Generales">
            <textarea style={ta} value={f.observacionesGenerales} onChange={set('observacionesGenerales')} placeholder="Sin novedad" />
          </Field>
          <PhotoField id="fotoRonda" label="Foto de la Ronda" value={fotoRonda} onChange={setFotoRonda} />
